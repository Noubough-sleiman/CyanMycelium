...
namespace CyanMycelium
{
  ...
  template <typename T>\
    void OP_FUNC_NAME(fname) (TensorPtr x, TensorPtr out, UnaryOperatorPtr node)\
    {\
  ...
  #define UNARY_FUNCTION_PTR(fname,type) &##OP_FUNC_NAME(fname,type)<type>
  ...
  template <typename T>\
    void OP_FUNC_NAME(fname) (TensorPtr x, TensorPtr y, TensorPtr out, UnaryOperatorPtr node)\
    {\
      ...
      {\
        ...
        {\
          T a = xdata[i];\
          T b = ydata[i];\
          res[i] = fname##_CODE(a,b,node);\
        }\
      }\
    }; 

  #define BINARY_FUNCTION_PTR(fname,type) &##OP_FUNC_NAME(fname,type)<type>
